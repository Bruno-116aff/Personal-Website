...
const totalJavaScriptGzipBytes = (await Promise.all(javaScriptAssets.map(gzipSize)))
  .reduce((total, size) => total + size, 0);
const totalCssGzipBytes = (await Promise.all(cssAssets.map(gzipSize)))
  .reduce((total, size) => total + size, 0);

if (totalJavaScriptGzipBytes > 75 * 1024) {
  errors.push(`client JavaScript exceeds the 75 KiB gzip budget (${totalJavaScriptGzipBytes} bytes)`);
}
