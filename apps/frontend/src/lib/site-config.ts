...
export function resolveSiteOrigin(value: string | undefined, options: SiteOriginOptions = {}) {
  ...
  const isLocalHttp =
    options.allowLocalHttp && parsed.protocol === 'http:' && isLocalHostname(parsed.hostname);
  ...
  if (
    ...
    throw new Error(
      'VITE_SITE_URL must contain only the site origin, without credentials, path, query or hash.',
    );
  }
