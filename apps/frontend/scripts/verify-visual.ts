  ...
  try {
    for (const viewport of viewports) {
      ...
      await referencePage.screenshot({
        path: resolve(
          artifactDirectory,
          `reference--${viewport.name}-${viewport.width}x${viewport.height}.png`,
        ),
        fullPage: true,
      });
      ...
      for (const route of liveRoutes) {
        ...
        if (response && response.status() >= 500)
          errors.push(`${route}: server returned ${response.status()}`);
        ...
        if (Math.max(overflow.body, overflow.document) > overflow.viewport + 1) {
          errors.push(
            `${route} at ${viewport.width}px: horizontal overflow (${Math.max(overflow.body, overflow.document)}px > ${overflow.viewport}px)`,
          );
        }
        ...
        await page.screenshot({
          path: resolve(
            artifactDirectory,
            `${routeFileName(route)}--${viewport.name}-${viewport.width}x${viewport.height}.png`,
          ),
          fullPage: true,
        });
        await page.close();
...
for (const route of ['reference', ...liveRoutes])
  console.log(`CAPTURED: ${route} at ${viewports.length} viewports`);
...
console.log(
  `PASS: visual review captured reference, ${liveRoutes.length - 1} public routes and 404 at 1440x960, 768x1024 and 390x844`,
);
