...
const totalJavaScriptGzipBytes = (await Promise.all(javaScriptAssets.map(gzipSize))).reduce(
  (total, size) => total + size,
  0,
);
const totalCssGzipBytes = (await Promise.all(cssAssets.map(gzipSize))).reduce(
  (total, size) => total + size,
  0,
);

if (totalJavaScriptGzipBytes > 75 * 1024)
  errors.push(
    `client JavaScript exceeds the 75 KiB gzip budget (${totalJavaScriptGzipBytes} bytes)`,
  );
if (totalCssGzipBytes > 10 * 1024)
  errors.push(`CSS exceeds the 10 KiB gzip budget (${totalCssGzipBytes} bytes)`);

for (const route of routes) {
  const html = await readFile(resolve('dist', route), 'utf8');
  const prerenderedRoot = html.match(/<div id="root">([\s\S]*)<\/div>\s*<\/body>/i)?.[1] ?? '';
  if (prerenderedRoot.replace(/<[^>]+>/g, '').trim().length < 100)
    errors.push(`${route}: primary content is not meaningfully prerendered`);
  if (/<(?:video|canvas|iframe)\b/i.test(html))
    errors.push(`${route}: contains a heavy visual runtime element`);
  if (/<img(?![^>]*(?:\bwidth=|\bheight=))[^>]*>/i.test(html))
    errors.push(`${route}: image is missing explicit dimensions`);
  if (/@import\s+url\(|fonts\.(?:googleapis|gstatic)\.com/i.test(html))
    errors.push(`${route}: requests a remote web font`);
}
...
if (
  /@import\s+url\(|fonts\.(?:googleapis|gstatic)\.com|https?:\/\/[^\s"')]+\.(?:woff2?|woff|ttf|otf)(?:\?|['")])/i.test(
    styles,
  )
) {
  errors.push('CSS requests a remote font');
}
if (!styles.includes('font-synthesis: none'))
  errors.push('CSS does not prevent synthetic font shifts');
...
for (const block of fontFaceBlocks) {
  ...
  if (!/font-display\s*:\s*optional\s*;/i.test(block))
    errors.push('local font face must use font-display: optional');
  const sources = [...block.matchAll(/url\(\s*["']?([^)"']+)["']?\s*\)/gi)].map(
    (match) => match[1],
  );
  if (
    sources.length === 0 ||
    sources.some((source) => !/\.woff2(?:\?|$)/i.test(source) || /^(?:https?:)?\/\//i.test(source))
  ) {
...
if (dependencies.some((dependency) => !allowedDependencies.has(dependency)))
  errors.push('production dependencies include an unreviewed client library');

const shareAssets = await readdir(resolve('dist', 'images', 'share'));
for (const asset of shareAssets.filter((file) => file.endsWith('.png'))) {
  if ((await stat(resolve('dist', 'images', 'share', asset))).size === 0)
    errors.push(`share asset is empty: ${asset}`);
}
